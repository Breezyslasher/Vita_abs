/*
    VitaABS — updater stub

    A tiny standalone Vita app whose ONLY job is to install a VitaABS update
    while VitaABS itself is closed, then relaunch it. This is the AutoPlugin2
    technique: a title cannot promote over itself (the Vita installer returns
    0x80101114 "in use" for the running app), so VitaABS installs and launches
    THIS stub, which — now that VitaABS is no longer the running title — can
    promote VitaABS's downloaded VPK cleanly, then boots VitaABS.
...
void render() {
    ...
    if (!g_ui.version.empty()) {
        ...
        drawWrapped(46, 294, 308, 0.85f, C_MUTED,
                    "Installing the update. VitaABS will restart automatically "
...
int main(int, char*[]) {
    ...
    // Give the shell a moment to finish tearing down VitaABS so the promoter
    ...
    if (rc == 0) {
        ...
        g_ui.note = "Update kept at ux0:data/VitaABS/update.vpk - install it with VitaShell";
        render();
        sceKernelDelayThread(4 * 1000 * 1000);
    }

    // Relaunch VitaABS either way — the updated build on success, the
