/*
    VitaABS — PS4 in-app self-update
    ...
    VitaABS itself. PS4-Store clears this by uninstalling the title and
...
// instantly (it took VitaABS down with CE-34878-0 when sceAppInstUtilAppExists
...
int installPkg(const std::string& pkgPath, const std::string& contentName, std::string& err) {
    ...
    // title is the running VitaABS itself. We deliberately do NOT uninstall
...
int installUpdaterApp(std::string& err) {
    // The helper's pkg ships inside VitaABS's own pkg (see CMakeLists), so the
    ...
    // VitaABS, so this install is not blocked by 0x80990088.
    const std::string src = "/app0/updater.pkg";
    const std::string dst = "/data/VitaABS/updater.pkg";
    ...
    return installPkg(dst, "VitaABS Updater", err);
}
