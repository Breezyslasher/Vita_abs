/**
 * VitaABS - In-app updates implementation
 *
 * Ported from pleNx (thcolin/gamepad-media-center-aggregator,
 * app/src/utils/version.cpp) onto VitaABS's HttpClient and dialogs.
...
constexpr const char* kRepo = "Breezyslasher/Vita_abs";
...
// Viewport helpers: VitaPlex exposed these through its platform layer; the
// VitaABS platform layer doesn't, so read borealis' content metrics directly.
inline float viewportWidth()  { return brls::Application::contentWidth; }
inline float viewportHeight() { return brls::Application::contentHeight; }
inline bool  isPortrait()     { return viewportHeight() > viewportWidth(); }
...
// the helper's uninstall always lands on an already-gone VitaABS. The update
...
void startInstall(const ReleaseInfo rel) {
    ...
    const float screenW = viewportWidth();
    float panelW = 388.0f;
    ...
    auto* keepOpen = makeLabel("Keep VitaABS open until this finishes", 11.0f, tok::disabled());
    ...
    asyncRun([rel, ui]() {
        ...
        for (int attempt = 0; attempt < 2 && !s_cancel.load(); attempt++) {
            ...
            // VitaABS's HttpClient doesn't surface a transport-error string;
        ...
        // install.
        {
            ...
            if (ps4::isUpdaterInstalled() && ps4::launchUpdater(err) == 0) {
                ...
                // so this is the normal path). Its pkg ships inside VitaABS's
                ...
                if (ps4::installUpdaterApp(setupErr) == 0) {
                    ...
                    if (launched) {
                        ...
                        finishInstall(ui, [path]() {
                            auto* d = new brls::Dialog(
                                ...
                                "or close VitaABS and install that .pkg with "
                    ...
                    finishInstall(ui, [path, setupErr]() {
                        auto* d = new brls::Dialog(
                            ...
                            "VitaABS and install that .pkg with Itemzflow (or "
        ...
        if (target.size() < 4 || target.compare(target.size() - 4, 4, ".nro") != 0)
            target = platformPath("VitaABS.nro");
        std::error_code ec;
        ...
        // fine), launches it, and quits; with VitaABS closed the stub
        // promotes the downloaded update.vpk and relaunches VitaABS.
        brls::sync([ui]() {
            if (!ui->dismissed->load())
...
// VitaABS notes are hand-written markdown with a fixed shape: an H1
...
void showNotesSheet(const ReleaseInfo rel) {
    ...
    const float screenW = viewportWidth();
    const float screenH = viewportHeight();
    float panelW = 620.0f;
...
void offerUpdate(const ReleaseInfo rel) {
    ...
    auto* sub = makeLabel("A new version of VitaABS is ready to install.", 11.5f, tok::muted());
...
void checkForUpdates(bool manual) {
    ...
    asyncRun([manual]() {
        ...
        // only needed while an update installs, and VitaABS reinstalls it on
        ...
        HttpClient client;
        HttpRequest req;
        // VitaABS publishes pre-releases; /releases/latest never returns
        ...
        if (!isNewer(rel.tag, current)) {
            ...
            if (manual) {
                brls::sync([current]() {
                    auto* d = new brls::Dialog("VitaABS is up to date (" + current + ").");
                    d->addButton("OK", []() {});
                    d->open();
                });
            }
