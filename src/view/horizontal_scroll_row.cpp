/**
 * VitaABS - Horizontal Scroll Row implementation
 */
...
brls::View* HorizontalScrollRow::getNextFocus(brls::FocusDirection direction, brls::View* currentView) {
    // Get the default next focus from borealis. When the row is at a
    // boundary this returns whatever the parent chain offers (the sidebar
    // for LEFT off the first cell) instead of trapping focus in the row.
