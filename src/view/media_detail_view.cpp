...
void MediaDetailView::startDownloadAndPlay(const std::string& itemId, const std::string& episodeId,
    ...
    // If not download-only, play the content
    if (!downloadOnly) {
#ifdef __vita__
        // On Vita, mpv cannot stream HTTP URLs directly - it crashes.
        // Download to a temp file first, then play the local file.
        brls::Logger::info("Vita: Downloading to temp file before playback");
        ...
        asyncRun([progressDialog, itemId, episodeId, title, duration, requestedStartTime]() {
            AudiobookshelfClient& client = AudiobookshelfClient::getInstance();
            ...
            // Use session's current time if we didn't get it from server
            if (startTime < 0) {
                startTime = session.currentTime;
            }

            brls::Logger::info("Session started: {} tracks, startTime={}s", session.audioTracks.size(), startTime);
            ...
            if (streamUrl.empty()) {
                ...
                brls::sync([progressDialog]() {
                    progressDialog->setStatus("Failed to get stream URL");
                    brls::delay(2000, [progressDialog]() { progressDialog->dismiss(); });
                });
                return;
            }
            ...
            if (!session.audioTracks.empty() && !session.audioTracks[0].mimeType.empty()) {
                const std::string& mime = session.audioTracks[0].mimeType;
                if (mime.find("mp4") != std::string::npos || mime.find("m4a") != std::string::npos ||
                    mime.find("m4b") != std::string::npos) {
                    ext = ".m4a";
                } else if (mime.find("flac") != std::string::npos) {
                    ext = ".flac";
                } else if (mime.find("ogg") != std::string::npos) {
                    ext = ".ogg";
                }
            }

            // Download to a simple temp file (overwritten each time, no caching)
            std::string tempPath = "ux0:data/VitaABS/temp_stream" + ext;
            ...
            SceUID fd = sceIoOpen(tempPath.c_str(), SCE_O_WRONLY | SCE_O_CREAT | SCE_O_TRUNC, 0777);
            if (fd < 0) {
                brls::Logger::error("Failed to create temp file: {}", tempPath);
                brls::sync([progressDialog]() {
                    progressDialog->setStatus("Failed to create temp file");
                    brls::delay(2000, [progressDialog]() { progressDialog->dismiss(); });
                });
                return;
            }
            ...
            bool success = httpClient.downloadFile(streamUrl,
                ...
                [&](int64_t size) {
                    totalSize = size;
                    brls::Logger::info("Stream file size: {} bytes", size);
                }
            );
            ...
            if (success) {
                brls::Logger::info("Stream download complete: {} ({} bytes)", tempPath, totalDownloaded);
                float seekTime = startTime;
                brls::sync([progressDialog, itemId, episodeId, tempPath, seekTime]() {
                    progressDialog->dismiss();
                    Application::getInstance().pushPlayerActivityWithFile(itemId, episodeId, tempPath, seekTime);
                });
            } else {
        ...
        return;
#else
        // On non-Vita platforms, mpv can stream HTTP URLs directly
