/*
    VitaABS — PS4 updater helper

    A tiny standalone PS4 app whose ONLY job is to install a VitaABS update
    while VitaABS itself is closed. This is the PS4 analogue of the Vita
    ...
    in the notifications / Downloads list, and the user reopens VitaABS once it
...
// Relaunching VitaABS when the install finishes. Layout per idc's
...
const char* kPkgPath  = "/data/VitaABS/update.pkg";
...
void uiInit() {
    ...
    // borealis demonstrably renders on this hardware (VitaABS logs
    ...
    g_win = SDL_CreateWindow("VitaABS Updater", SDL_WINDOWPOS_UNDEFINED,
...
int main(int, char*[]) {
    ...
    // nothing. VitaABS makes these same calls successfully because borealis
    // has already run this.
    int sdl = SDL_Init(SDL_INIT_VIDEO);
    vlog("ps4 updater: SDL_Init -> %d (%s)", sdl, sdl == 0 ? "ok" : SDL_GetError());

    // Give VitaABS a moment to fully exit before we uninstall its title.
    sleep(3);
    ...
    // launched by a quitting VitaABS never becomes the foreground/composited
    ...
    // the same way for a sandboxed app, and VitaABS only gets away with direct
    ...
    // Optional extras: used to wait for the install and relaunch VitaABS. If
    ...
    paramsEx.param.contentName      = "VitaABS";
    ...
    // console with no VitaABS at all — which is exactly what happened when the
    ...
    // VitaABS, after proving on-device that both fight the platform:
    ...
    // VitaABS removes this helper on its next start (ps4::removeUpdaterApp).
    sleep(2);
    vlog("ps4 updater: done (install continues in the background; reopen "
         "VitaABS when the system notification finishes)");
    return 0;
}
