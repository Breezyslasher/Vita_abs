...
///
/// Returns 0 if the helper was launched (VitaABS should now quit); non-zero
...
/// Install the updater helper app, whose pkg ships inside VitaABS's own pkg
...
/// Remove the updater helper if it's installed. Call this from VitaABS once an
/// update has completed — the helper is only needed while one installs, and
/// VitaABS reinstalls it on demand. Cleaning up from here rather than letting
